...
class MemoryManager:
    ...
    def _migrate(self, conn: sqlite3.Connection, from_version: int) -> None:
        ...
        # No migrations needed yet — placeholder for future use.
        conn.execute(
    ...
    def get_context_window(
        ...
        """
        Return the most recent ``limit`` entries for a task.

        Returns a list of dicts with keys: role, content, metadata.
        Shape matches the existing MemoryManager.get_context_window() output.
        """
        conn = self._connect()
        try:
            rows = conn.execute(
