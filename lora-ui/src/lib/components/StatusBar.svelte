...
<header class="statusbar" aria-label="System status">
  ...
  <div class="statusbar-right">
    ...
    <!-- Streaming indicator -->
    {#if streaming}
      <div class="status-chip streaming-chip" title="Streaming response">
        <span class="stream-icon" aria-hidden="true">
          <svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor">
            <circle cx="4"  cy="12" r="3"><animate attributeName="opacity" values="1;0.2;1" dur="1.2s" begin="0s"    repeatCount="indefinite"/></circle>
            <circle cx="12" cy="12" r="3"><animate attributeName="opacity" values="1;0.2;1" dur="1.2s" begin="0.4s"  repeatCount="indefinite"/></circle>
            <circle cx="20" cy="12" r="3"><animate attributeName="opacity" values="1;0.2;1" dur="1.2s" begin="0.8s"  repeatCount="indefinite"/></circle>
          </svg>
        </span>
        <span>streaming</span>
      </div>
    ...
    {#if $agents.loaded && $agents.agents.length > 0}
      <div class="status-chip" title="Active agents: {$agents.agents.join(', ')}">
