...
| 16 | [Runtime Backend Layer](docs/architecture/16-runtime-backend-layer.md) (`BaseRuntimeClient`) | Authoritative | 2026-07-14 | The structural-typing `BaseRuntimeClient` protocol and `runtime_factory.create_runtime()` swap-only backend selection; covers the oMLX, MLX-LM/Foundry, and Ollama backends. Includes §16.3, a scoped-but-not-built proposal for making the Settings UI's Runtime Backend control (currently a display preference, §7.10) a genuine live switch; and §16.4, `OllamaRuntimeClient.embed()` implemented (real `/api/embed`, no longer a stub), the three-tier runtime-backend/MLX/keyword-only embedding-source precedence (platform-gated: MLX `EmbeddingEngine` only attempted on Apple Silicon), and a chat-model fail-fast `ValueError` replacing an unsafe hardcoded default — live-verified end to end with a cloud chat model and a local embedding model sharing one Ollama daemon. |
