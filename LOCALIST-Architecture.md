...
| 16 | [Runtime Backend Layer](docs/architecture/16-runtime-backend-layer.md) (`BaseRuntimeClient`) | Authoritative | 2026-07-13 | The structural-typing `BaseRuntimeClient` protocol and `runtime_factory.create_runtime()` swap-only backend selection; covers the oMLX, MLX-LM/Foundry, and Ollama backends. Now includes §16.3: a scoped-but-not-built proposal for making the Settings UI's Runtime Backend control (currently a display preference, §7.10) a genuine live switch, and why it requires rebuilding `WikiAgent`/`ConversationalAgent`/`ControllerAgent` rather than a one-line state mutation. |
