...
| 17 | [WikiAgent Diff-Target Path](docs/architecture/17-wiki-agent-diff-target.md) | Authoritative | 2026-07-09 | Standalone `diff_target` context field letting `wiki_agent` propose/apply a diff against an existing wiki page with no raw file involved; Planner Priority 1b routing (reusing P3c's `resolve_graph_target()`); the content-based `apply_unified_diff` hunk-location fix; and a review-then-apply chat UI (`POST /wiki/apply-diff`, `metadata.pending_diffs`) letting a user approve a proposed diff before it writes — live-verified end to end in a real browser, including a live corruption-and-repair. Open items: no wiki write-safety net (`wiki/` is gitignored, now higher priority after the live corruption), a confirmed-safe bullet/diff-marker edge case on context lines. |
